...
export default function SignupForm() {
  ...
  const handleSignup = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // Step 1: Verify OTP - this creates the phone-authenticated user
      const userCredential = await confirmationResult.confirm(formData.otp);
      const user = userCredential.user;
      
      setPhoneVerified(true);
      
      // Step 2: Link email/password credential to the same user
      const email = `${formData.phone}${FAKE_EMAIL_DOMAIN}`;
      const emailCredential = EmailAuthProvider.credential(email, formData.password);
      
      // Link the email credential to the existing phone-authenticated user
      ...
      if (error.code === 'auth/invalid-verification-code') {
        ...
        errorMessage = "This account is already linked to an email/password. Please try logging in.";
      } else if (error.message) {
        errorMessage = error.message;
      }
